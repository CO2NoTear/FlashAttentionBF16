...
echo "[1/3] Building FP32/FP32-OPT/BF16 demos..."
...
printf "%-10s %-8s %-14s %-14s %-14s %-10s\n" "N" "D" "FP32(ms)" "FP32_OPT(ms)" "BF16(ms)" "FP32/BF16"
...
PY
  ...
  printf "%-10s %-8s %-14.6f %-14.6f %-14.6f %-10s\n" "$n" "$d" "$fp32_avg" "$fp32_opt_avg" "$bf16_avg" "$speedup"
...
PY
  ...
  if [[ "$better" == "1" ]]; then
    ...
    best_fp32="$fp32_avg"
    best_fp32_opt="$fp32_opt_avg"
    best_bf16="$bf16_avg"
  fi

  rm -f "$fp32_log" "$fp32_opt_log" "$bf16_log"
done
...
if [[ "${is_advantage}" == "1" ]]; then
  ...
  echo "FP32_OPT avg: ${best_fp32_opt} ms"
