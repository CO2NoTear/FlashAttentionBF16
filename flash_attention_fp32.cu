...
__global__ void pv_matmul_kernel(const float* probs, const float* v, float* out,
                                 int n, int d) {
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
...
int main(int argc, char** argv) {
    ...
    dim3 block_mm(16, 16);
    dim3 grid_qk((n + block_mm.x - 1) / block_mm.x,
                 (n + block_mm.y - 1) / block_mm.y);
    dim3 grid_pv((d + block_mm.x - 1) / block_mm.x,
                 (n + block_mm.y - 1) / block_mm.y);
    ...
    for (int i = 0; i < warmup_iters; ++i) {
        qk_matmul_kernel<<<grid_qk, block_mm>>>(d_hidden, d_hidden, d_scores, n, d, scale);
        softmax_rows_kernel<<<grid_softmax, block_softmax>>>(d_scores, d_probs, n);
        pv_matmul_kernel<<<grid_pv, block_mm>>>(d_probs, d_hidden, d_out, n, d);
    }
    ...
    CHECK_CUDA(cudaMemcpy(h_out.data(), d_out, n * d * sizeof(float),
                          cudaMemcpyDeviceToHost));

    if (print_output) {
        print_tensor(h_out, n, d, "FP32 output tensor");
    }
    std::cout << "N=" << n << "\n";
    std::cout << "D=" << d << "\n";
    std::cout << "FP32_TOTAL_MS=" << total_ms << "\n";
    std::cout << "FP32_AVG_MS=" << avg_ms << "\n";
    std::cout << "FP32_ITERS=" << bench_iters << "\n";
